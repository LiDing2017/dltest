...
class AddEventTest(unittest.TestCase):
    ...
    def test_add_event_request_error(self):
        ...
        self.assertEqual(result['status'],'request error')
    ...
    def test_add_event_sign_null(self):
        ...
        self.assertEqual(result['status'], 'user sign null')
    def test_add_event_time_out(self):
        '''请求超时'''
        now_time=str(int(self.client_time)-61)
        payload={'eid':1,'name':'','limit':'','address':'','start_time':'','time':now_time,'sign':'abc'}
        r = requests.post(self.base_url,data=payload)
        result = r.json()
        self.assertEqual(result['status'], 10013)
        self.assertEqual(result['status'], 'user sign timeout')
    def test_add_event_sign_error(self):
        '''签名错误'''
        payload={'eid':1,'name':'','limit':'','address':'','start_time':'','time':self.client_time,'sign':'abc'}
        r = requests.post(self.base_url,data=payload)
        result = r.json()
        self.assertEqual(result['status'], 10014)
        self.assertEqual(result['status'], 'user sign error')
    def test_add_event_success(self):
        '''添加成功'''
        payload={'eid':21,'name':'一加5手机发布会','limit':2000,'address':'深圳宝体',
                 'start_time':'2010-05-10 12:00:00','time':self.client_time,'sign':self.sign_md5}
        r = requests.post(self.base_url,data=payload)
        result = r.json()
        self.assertEqual(result['status'], 200)
        self.assertEqual(result['status'], 'add event success')
