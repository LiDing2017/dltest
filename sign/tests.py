...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=4,name='oneplus 3 event',status=True,limit=2000,address='shenzhen',start_time='2016-08-31 02:18:22')
        Guest.objects.create(id=1,event_id=4,realname='alen',phone='13711001101',email='alen@mail.com',sign=False)
...
class LoginActionTest(TestCase):
    '''测试登录动作'''
    def setUp(self):
        User.objects.create_user('admin','admin@mail.com','admin123321')
    ...
    def test_login_action_sucess(self):
        '''登陆成功'''
        test_data={'username':'admin','password':'admin321123'}
        response=self.client.post('/login_action/',data=test_data)
...
class EventMangeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.login_user={'username':'admin','password':'admin123321'}
    ...
    def test_event_manage_sreach_sucess(self):
        ...
        self.assertEqual(response.status_code,200)
        self.assertIn(b'xiaomi5',response.content)
        self.aasertIn(b'beinging',response.content)
class EventMangeTest(TestCase):
    '''嘉宾管理'''
    def setUp(self):
        User.objects.create_user('admin','admin@mail.com','admin123321')
    ...
    def test_guest_manage_sreach_sucess(self):
        '''测试嘉宾搜索'''
        response=self.client.post('/login_action/',data=self.login_user)
        response=self.client.post('/search_phone/',{'phone':'18611001100'})
        self.assertEqual(response.status_code,200)
        self.assertIn(b'alen',response.content)
        self.aasertIn(b'18611001100',response.content)
class SignIndexActionTest(TestCase):
    '''发布会签到'''
    def setUp(self):
        User.objects.create_user('admin','admin@mail.com','admin123321')
        Event.objects.create(id=1,name='xiaomi5',limit=2000,address='beijing',status=1,start_time='2017-8-10 12:30:00')
        Event.objects.create(id=2,name='oenplus4',limit=2000,address='shenzhen',status=1,start_time='2017-6-10 12:30:00')
        Guest.objects.create(realname='alen',phone=18611001100,email='alen@mail.com',sign=0,event_id=1)
